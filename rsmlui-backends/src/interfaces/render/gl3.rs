...
pub struct RendererGl3(pub(crate) BorrowedInterface<'static, RmlRenderInterface>);

not_send_sync!(RendererGl3);

impl RendererGl3 {
    pub fn new() -> Self {
        // Safety: `new_gl3_render_interface` returns a valid, non-null pointer allocated
        // with `new`. The `'static` lifetime is correct because the allocation has no
        // lifetime tied to any other resource.
        Self(unsafe { BorrowedInterface::new(new_gl3_render_interface()) })
    }
}

impl Default for RendererGl3 {
    fn default() -> Self {
        Self::new()
    }
}
